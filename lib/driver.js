...
class XCUITestDriver extends BaseDriver {
  ...
  async start () {
    ...
    log.info(`Determined device to run tests on: udid: '${udid}', real device: ${realDevice}`);
    ...
    let isLogCaptureStarted = false;
    startLogCapture()
      .then((started) => isLogCaptureStarted = started) // eslint-disable-line promise/prefer-await-to-then
      .catch((err) => { // eslint-disable-line promise/prefer-await-to-callbacks
        log.warn(`Log capture failed: ${err.message}`);
      });
    ...
    if (this.isSimulator()) {
      ...
      if (!isLogCaptureStarted) {
        ...
        startLogCapture().catch((err) => { // eslint-disable-line promise/prefer-await-to-callbacks
          log.warn(`Log capture failed: ${err.message}`);
        });
      }
    }
